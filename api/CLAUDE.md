...
`RequestIdMiddleware` is pure ASGI on purpose: `BaseHTTPMiddleware` wraps the app in an `anyio` task group → asyncpg raises `Future attached to a different loop`. **New middleware must be pure-ASGI.** Request id = uuid4; client `X-Request-Id` honored only if a valid uuid4, else replaced; on every response (incl. errors) and bound into every log line of the request. Order, outermost first: `ServerErrorMiddleware → CORS → RequestIdMiddleware → RequestContextMiddleware → ExceptionMiddleware (Starlette) → router`. `RequestContextMiddleware` is added **first** so it is innermost and can read the request id; it clears + binds structlog contextvars (`request_id`; `current_user` binds `user_id`) and writes exactly one `http.request` line — route **template**, status, `duration_ms`, `user_id`, never the query string or raw path (≥500 ERROR, ≥400 WARNING, probes DEBUG). It must not clear context on exit: the unhandled-exception handler runs after it returns. `RequestContextMiddleware` also records `http_requests_total{method,status}` + `http_request_duration_seconds{method,route}` at the same point as the access line (metrics declared only in `jobify.observability.metrics`); CORS stays outermost, so preflight (OPTIONS) short-circuits aren't counted. **Ordering is load-bearing:** the `http.request` log line is written BEFORE the metric calls, so an unexpected metrics failure never costs the access line. The `method` metric label is bounded (`{GET,HEAD,POST,PUT,PATCH,DELETE,OPTIONS,OTHER}`, via `_metric_method`) — the log keeps the real, unbounded method. `create_app()` calls `ensure_multiprocess_dir_ready()` right after `configure_logging` so a `PROMETHEUS_MULTIPROC_DIR` pointing at a missing directory fails at boot, not on the first request. `CORSMiddleware` mounted last (outermost). Origins from `JOBIFY_CORS_ALLOW_ORIGINS` (default `http://localhost:8080`); no cookies → `allow_credentials` off. Only web needs it (mobile sends no `Origin`).
...
- **The `applicant_preferences` row is eagerly created at signup** (`AuthService._upsert_identity`, like consent seeding). GET/PATCH `/v1/applicants/me/preferences` go through `_require_preferences_row`, which distinguishes two cases: **no row at all** (pre-0021 accounts — 0021 never backfilled, 0028 does — or seeded applicants) → provision the default row (`INSERT … SELECT` from the **live** applicant — a DSR erasure committing after `require_applicant` hard-deletes prefs and soft-deletes the applicant, and a bare `VALUES` insert would resurrect a row PATCH then fills with location/CTC; plus `ON CONFLICT DO NOTHING` on the live partial-unique index, since the client fires concurrent first reads) and log `preferences.row-provisioned`; **only a soft-deleted row** → invariant violation, 500 `applicant_preferences_missing` (never resurrected). Migration 0028 uses the same "never existed" rule.
...
- **Feed filters (spec `2026-07-31-feed-filters-search-design.md`):** optional `q`/`location`(repeatable)/`min_years`/`min_ctc` narrow the same statement — never the ordering. `min_ctc` keeps `ctc_max IS NULL` jobs visible (undisclosed pay ≠ low pay). The cursor payload's `f` field is the canonicalized filter-set hash; mismatch with the request's params → `400 invalid_cursor` (an f-less cursor is the "no filters" case, which also keeps pre-deploy cursors valid). Normalize params (trim, drop empties) BEFORE `filters_hash` — hashing raw input would make equal filter sets produce different hashes. Bounds: `q`≤100 chars, ≤20 `location` items (each 1–100), `min_years`≤80. **`q` is the API's only free-text query param** — it must never reach logs: the API's own access line logs the route template only. `uvicorn.access` is disabled in code by `configure_logging` (see `core/CLAUDE.md`), so it never writes the raw request line with `q` in it; `--no-access-log` is kept as belt-and-braces, not the mechanism that stops it. **`min_ctc` is the API's only `Decimal` query param** — OpenAPI renders it `anyOf number|string`; hand-written clients send a number, schema-codegen clients must pick a branch.
...
- **`PATCH /v1/jobs/{id}/applications/{aid}/stage`** — recruiter pipeline; bad stage values 422 at body validation before any lookup (`Literal` of the 5 targets; `applied` never recruiter-settable), then `_load_recruiter_job` guard, then 404 (application under this job, uniform) → 409 `application_withdrawn`. Transition side-effects live in `jobify_api.applications.service.change_application_stage` (one txn: structlog → audit → event → dual-channel notification). **`GET /v1/applications/{id}/timeline`** is applicant-owned (uniform 404) and never exposes the actor.
