# api/src/kpa/db/migrations/versions/0019_jobs_recruiter_list_index.py
...
def upgrade() -> None:
    op.execute(
        ...
        "ON kpa.jobs (employer_id, posted_at DESC, id DESC) "
        "WHERE deleted_at IS NULL"
    )
    op.execute("DROP INDEX IF EXISTS kpa.ix_jobs_employer_id_live")


def downgrade() -> None:
    op.execute(
        "CREATE INDEX ix_jobs_employer_id_live "
        "ON kpa.jobs (employer_id) "
        "WHERE deleted_at IS NULL"
    )
    op.execute("DROP INDEX IF EXISTS kpa.ix_jobs_employer_posted_at_live")
