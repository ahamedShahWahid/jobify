# api/src/kpa/db/migrations/versions/0018_employer_invites.py
...
Adds:
- kpa.employer_invite_status ENUM ('pending', 'accepted', 'revoked', 'expired')
- kpa.employer_invites table (soft-delete trio, FKs, role CHECK)
...
def upgrade() -> None:
    ...
    op.create_table(
        ...
        sa.Column(
            "employer_id",
            postgresql.UUID(as_uuid=True),
            sa.ForeignKey("kpa.employers.id"),
            nullable=False,
        ),
        ...
        sa.Column(
            "status",
            postgresql.ENUM(name="employer_invite_status", schema="kpa", create_type=False),
            nullable=False,
            server_default=sa.text("'pending'::kpa.employer_invite_status"),
        ),
        sa.Column(
            "invited_by_user_id",
            postgresql.UUID(as_uuid=True),
    ...
    op.execute(
        ...
        "ON kpa.employer_invites (employer_id, email) "
        "WHERE deleted_at IS NULL AND status = 'pending'"
    )
    # Invitee lookup path: pending invites by email.
    op.execute(
        "CREATE INDEX ix_employer_invites_email_live "
        "ON kpa.employer_invites (email) "
        "WHERE deleted_at IS NULL AND status = 'pending'"
    )


def downgrade() -> None:
    op.execute("DROP INDEX IF EXISTS kpa.ix_employer_invites_email_live")
    op.execute("DROP INDEX IF EXISTS kpa.ix_employer_invites_pending_live")
    op.drop_table("employer_invites", schema="kpa")
    postgresql.ENUM(
        "pending",
        "accepted",
        "revoked",
