...
Adds:
- jobify.matches (one row per (applicant_id, job_id) live pair, JSONB metadata)
...
def upgrade() -> None:
    ...
    op.execute(
        ...
        "ON jobify.matches (applicant_id, total_score DESC) "
    ...
    op.execute(
        ...
        "ON jobify.matches (job_id, total_score DESC) "
        "WHERE deleted_at IS NULL AND surfaced_at IS NOT NULL"
    )


def downgrade() -> None:
    op.execute("DROP INDEX IF EXISTS jobify.ix_matches_job_surfaced")
    op.execute("DROP INDEX IF EXISTS jobify.ix_matches_applicant_surfaced")
    op.drop_index("ix_matches_applicant_job_live", table_name="matches", schema="jobify")
    op.drop_table("matches", schema="jobify")
