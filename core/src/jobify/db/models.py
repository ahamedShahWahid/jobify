"""SQLAlchemy declarative models for the Jobify service.
...
class Base(DeclarativeBase):
    """Declarative base. Lives in the `jobify` schema."""

    __table_args__: Any = {"schema": "jobify"}  # noqa: RUF012 — SQLAlchemy base declares this as Any
...
class RefreshToken(Base):
    ...
    replaced_by_id: Mapped[uuid.UUID | None] = mapped_column(
        UUID(as_uuid=True),
        ForeignKey("jobify.refresh_tokens.id", ondelete="SET NULL"),
        nullable=True,
    )
...
class Employer(Base):
    ...
    # Admin verification review (migration 0020). Tri-state is DERIVED:
    # verified_at set -> verified; else rejected_at set -> rejected; else pending.
    # Verify/reject are mutually exclusive — each clears the other's timestamp.
    rejected_at: Mapped[datetime | None] = mapped_column(DateTime(timezone=True), nullable=True)
    rejection_reason: Mapped[str | None] = mapped_column(Text, nullable=True)
    created_by_user_id: Mapped[uuid.UUID | None] = mapped_column(
        UUID(as_uuid=True),
        ForeignKey("jobify.users.id"),
        nullable=True,
    )
    created_at: Mapped[CreatedAt]
    updated_at: Mapped[UpdatedAt]
...
class EmployerUser(Base):
    ...
    employer_id: Mapped[uuid.UUID] = mapped_column(
        UUID(as_uuid=True),
        ForeignKey("jobify.employers.id"),
        nullable=False,
    )
    user_id: Mapped[uuid.UUID] = mapped_column(
        UUID(as_uuid=True),
...
class Job(Base):
    ...
    employer_id: Mapped[uuid.UUID] = mapped_column(
        UUID(as_uuid=True),
        ForeignKey("jobify.employers.id", ondelete="RESTRICT"),
        nullable=False,
    )
