...
def upgrade() -> None:
    ...
    op.execute(
        ...
        "ON jobify.user_consents (user_id, scope) WHERE deleted_at IS NULL"
    )
    ...
    bind.connection.dbapi_connection.run_async(
        lambda conn: conn.execute(
            "ALTER TYPE jobify.notification_status ADD VALUE IF NOT EXISTS 'cancelled'"
        )
    )
...
def downgrade() -> None:
    ...
    op.drop_column("notifications", "cancelled_at", schema="jobify")
    op.execute("DROP INDEX jobify.ix_user_consents_user_scope_live")
    op.drop_table("user_consents", schema="jobify")
