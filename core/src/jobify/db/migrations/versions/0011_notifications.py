...
Adds:
- jobify.notification_status ENUM ('pending', 'dispatching', 'sent', 'failed')
- jobify.notification_channel ENUM ('email', 'in_app')
- jobify.notifications table with soft-delete trio, JSONB payload, and two
...
def upgrade() -> None:
    ...
    op.create_table(
        ...
        sa.Column(
            "channel",
            postgresql.ENUM(name="notification_channel", schema="jobify", create_type=False),
            nullable=False,
        ),
        sa.Column(
            "status",
            postgresql.ENUM(name="notification_status", schema="jobify", create_type=False),
            nullable=False,
            server_default=sa.text("'pending'::jobify.notification_status"),
        ),
    ...
    op.execute(
        ...
        "ON jobify.notifications (status, send_after) "
    ...
    op.execute(
        ...
        "ON jobify.notifications (user_id, created_at DESC) "
        "WHERE deleted_at IS NULL"
    )


def downgrade() -> None:
    op.execute("DROP INDEX IF EXISTS jobify.ix_notifications_user_id_created_at_live")
    op.execute("DROP INDEX IF EXISTS jobify.ix_notifications_status_send_after_live")
    op.drop_table("notifications", schema="jobify")
    postgresql.ENUM(
        "pending",
        "dispatching",
        "sent",
