...
- **Worker runs from repo root, as two processes** (`-Q parse,embed,score` + `-Q notify,outbox`, each `--pool=solo`) — see `worker/README.md` for the commands and why. API/worker transactions stage task intents in `outbox_events`; `sweep_outbox` publishes them by task name.
