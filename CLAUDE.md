...
- **Every `tests/integration/test_*.py` needs module-level `pytestmark = pytest.mark.integration`** — CI's unit job (`-m "not integration"`) has NO Postgres; an unmarked test leaks into it and fails at connect. Per-test decorators drifted (5 strays); locally invisible because a dev DB is always up.
