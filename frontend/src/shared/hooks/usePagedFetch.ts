...
import { errorMessage } from "../api/transport";
...
/**
 * Cursor-pagination state machine shared by every list screen across surfaces
 * (web: feed / applied / saved / inbox; console: audit log, jobs, applicants,
 * verification). The caller supplies a `fetcher` (closing over client + filters)
 ...
 * if the caller flips a filter while a page is in flight, the slower response is
 * discarded instead of clobbering the newer one (a race the previous per-surface
 * hand-rolled copies all had).
 */
