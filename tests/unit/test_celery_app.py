...
def test_cleanup_refresh_tokens_is_beat_scheduled() -> None:
    entry = celery_app.conf.beat_schedule["cleanup-refresh-tokens"]
    assert entry["task"] == "jobify.cleanup_refresh_tokens"
    assert entry["schedule"].run_every.total_seconds() == 86400


def test_cleanup_refresh_tokens_routes_to_outbox_queue() -> None:
    assert celery_app.conf.task_routes["jobify.cleanup_refresh_tokens"] == {"queue": "outbox"}
